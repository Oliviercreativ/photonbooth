<template>
  <div class="photo-preview fixed inset-0 z-40 bg-black flex items-center justify-center">
    ...
    <button v-if="photos.length > 1" @click="$emit('navigate', 'prev')"
      class="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/20 rounded-full flex items-center justify-center text-gray-800 text-xl backdrop-blur z-10">
      ←
    </button>

    <button v-if="photos.length > 1" @click="$emit('navigate', 'next')"
      class="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/20 rounded-full flex items-center justify-center text-gray-800 text-xl backdrop-blur z-10">
      →
    </button>
    ...
    <div class="absolute top-0 left-0 right-0 p-4 bg-gradient-to-b from-black/50 to-transparent z-10">
      <div class="flex justify-between items-center text-gray-800">
        <div>
          ...
          <p class="text-sm text-gray-800/70">{{ formatFullDate(photo.timestamp) }}</p>
        </div>
    ...
    <div class="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/50 to-transparent">
      <div class="flex justify-center gap-4">
        <button @click="$emit('download', photo)"
          class="bg-blue-500 text-gray-800 px-4 py-2 rounded-full font-medium flex items-center gap-2">
          📥 Télécharger
        </button>
        <button @click="$emit('share', photo)"
          class="bg-green-500 text-gray-800 px-4 py-2 rounded-full font-medium flex items-center gap-2">
          📤 Partager
        </button>
        <button @click="$emit('delete', photo)"
          class="bg-red-500 text-gray-800 px-4 py-2 rounded-full font-medium flex items-center gap-2">
          🗑️ Supprimer
        </button>
      </div>
      ...
      <div v-if="photos.length > 1" class="text-center mt-3">
        <p class="text-gray-800/70 text-sm">
